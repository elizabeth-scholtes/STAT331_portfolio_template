---
title: "Challenge 2 - Lab 2: Exploring Rodents with ggplot2"
...
```{r data}
# Code for question 1! 
surveys <- read_csv(here("supporting_artifacts", "extended_learning", "Challenge 2","surveys.csv")) 
```
...
```{r}
dim(surveys)
#The dimensions of the surveys data is 30,463 rows by 15 columns 
```
...
```{r}
spec(surveys)
# There are three types of data: character, double, and date
```
...
ggplot(data = surveys, 
  ...
  geom_jitter(size = .25, color = "Purple", alpha = .25)
```
...
```{r}
# Loading in the ggridges package
library(ggridges)
```
